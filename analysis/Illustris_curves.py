...
neutral_gas = gas_fraction > 0.99 #what is the best limit?
...
def smoothing(group, circle_size): #do not smooth the gas
	...
	for i in range(len(star_xs)):
		c1 = SkyCoord(star_xs[i] / 13.67 / h, star_ys[i] / 13.67 / h, unit=(u.deg,u.deg)) 
		sep = c1.separation(c)
...
plt.savefig('/Volumes/FRIEND/analogs/plots/rcs/{}_rc.png'.format(halo), bbox_inches='tight')
plt.close()

#========================================================
single_plot()
plt.hist(star1_ad[:-1], bins=range(-100, 100, 8), label='Group 1: {} stars, AD ={}'.format(sum(group1), np.median(star1_ad[:-1])), normed=1, histtype='step', linewidth=1.6,linestyle='--',stacked=True,fill=False, color='b')
plt.hist(star2_ad[:-1], bins=range(-100, 100, 8), label='Group 2: {} stars, AD ={}'.format(sum(group2), np.median(star2_ad[:-1])), normed=1, histtype='step', linewidth=1.6,stacked=True,fill=False, color='m', hatch='//', alpha=0.4)
plt.hist(star3_ad[:-1], bins=range(-100, 100, 8), label='Group 3: {} stars, AD ={}'.format(sum(group3), np.median(star3_ad[:-1])), normed=1, histtype='step', linewidth=1.6,stacked=True,fill=True, color='green', alpha=0.4)
plt.hist(star4_ad[:-1], bins=range(-100, 100, 8), label='Group 4: {} stars, AD ={}'.format(sum(group4), np.median(star4_ad[:-1])), normed=1, histtype='step', linewidth=1.6,linestyle='-',stacked=True,fill=False, color='r')
plt.legend(loc=2, frameon=False)
plt.xlim(-100,100)
plt.xlabel(r'$ \rm Asymmetric\ Drift:\ \itv_{a}\ \rm(km\ s^{-1})$', fontsize=13)
plt.savefig('/Volumes/FRIEND/analogs/plots/hists/{}_AD.png'.format(halo), bbox_inches='tight')
...
centerx= 13
centery= -12
radius1= 200 / 60 / 60 * 13.67 #fix scaling factor
radius2= 275 / 60 / 60 * 13.67 #fix scaling factor
...
axes[0,0].scatter(star_x[group1], star_y[group1], c=star_vrad_all[group1], cmap='plasma', s=4, vmin=-150,vmax=100) 
axes[0,1].scatter(star_x[group2], star_y[group2], c=star_vrad_all[group2], cmap='plasma', s=4, vmin=-150,vmax=100) 
axes[0,2].scatter(star_x[group3], star_y[group3], c=star_vrad_all[group3], cmap='plasma', s=4, vmin=-150,vmax=100) 
im0=axes[0,3].scatter(star_x[group4], star_y[group4], c=star_vrad_all[group4], cmap='plasma', s=4, vmin=-150,vmax=100)
...
axes[1,0].scatter(star1x, star1y, c= star1_vrad_smoothed, cmap='plasma', s=4, vmin=-150,vmax=100) 
axes[1,1].scatter(star2x, star2y, c= star2_vrad_smoothed, cmap='plasma', s=4, vmin=-150,vmax=100)
axes[1,1].add_artist(c4)
axes[1,2].scatter(star3x, star3y, c= star3_vrad_smoothed, cmap='plasma', s=4, vmin=-150,vmax=100) 
axes[1,2].add_artist(c5)
im1=axes[1,3].scatter(star4x, star4y, c= star4_vrad_smoothed, cmap='plasma', s=4, vmin=-150,vmax=100)
...
axes[2,0].scatter(star1x, star1y, c= star1_disp, cmap='copper', s=4, vmin=0,vmax=100) 
axes[2,0].add_artist(c2)
axes[2,1].scatter(star2x, star2y, c= star2_disp, cmap='copper', s=4, vmin=0,vmax=100)
axes[2,1].add_artist(c1)
axes[2,2].add_artist(c3)
axes[2,2].scatter(star3x, star3y, c= star3_disp, cmap='copper', s=4,vmin=0,vmax=100) 
axes[2,3].add_artist(c7)
im2=axes[2,3].scatter(star4x, star4y, c= star4_disp, cmap='copper', s=4,vmin=0,vmax=100)
...
plt.savefig('/Volumes/FRIEND/analogs/plots/maps/{}_map.png'.format(halo), bbox_inches='tight')
